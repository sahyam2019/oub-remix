...
PM_PERMIT = bot.config['userbot'].getboolean('pm_permit', False)
redis = bot.database
...
@bot.onMessage(incoming=True, edited=False)
async def pm_incoming(event: NewMessage.Event) -> None:
    ...
    if count == 0:
        user = await get_chat_link(entity)
        await bot.delete_messages(input_entity, sent)
        await bot(functions.messages.ReportSpamRequest(peer=input_entity))
        await bot(functions.contacts.BlockRequest(id=input_entity))
        await event.answer(blocked, log=('pmpermit', blocklog.format(user)))
        spammers.pop(sender, None)
        return

    if not lastmsg:
        result = await bot.get_messages(input_entity, reverse=True, limit=1)
        if result[0].id == event.id:
            new_pm = True
    if lastoutmsg:
        await bot.delete_messages(input_entity, [lastoutmsg])
    lastoutmsg = None
...
@bot.onMessage(outgoing=True, edited=False)
async def pm_outgoing(event: NewMessage.Event) -> None:
    ...
    result = await bot.get_messages(await event.get_input_chat(),
                                       reverse=True,
                                       limit=1)
    if result[0].out:
...
@bot.onMessage(command=("approve", plugin_category),
                  outgoing=True,
                  regex=r"approve(?: |$)(.+)?$")
async def approve(event: NewMessage.Event) -> None:
    ...
    if user:
        ...
        if user.id in spammers:
            ...
            await bot.delete_messages(user, sent)
            await bot(
                functions.account.UpdateNotifySettingsRequest(
                    peer=user.id, settings=DEFAULT_UNMUTE_SETTINGS))


@bot.onMessage(command=("unapprove", plugin_category),
                  outgoing=True,
...
@bot.onMessage(command=("block", plugin_category),
                  outgoing=True,
                  regex=r"block(?: |$)(.+)?$")
async def block(event: NewMessage.Event) -> None:
    ...
    if user:
        href = await get_chat_link(user)
        try:
            result = await bot(functions.contacts.BlockRequest(id=user.id))
        except Exception:
            pass
        if result:
...
@bot.onMessage(command=("unblock", plugin_category),
                  outgoing=True,
                  regex=r"unblock(?: |$)(.+)?$")
async def unblock(event: NewMessage.Event) -> None:
    ...
    if user:
        href = await get_chat_link(user)
        try:
            result = await bot(functions.contacts.UnblockRequest(id=user.id)
                                  )
        except Exception:
            pass
        if result:
...
@bot.onMessage(command=("approved", plugin_category),
                  outgoing=True,
                  regex=r"approved$")
...
async def get_user(event: NewMessage.Event) -> types.User or None:
    ...
    if match:
        ...
        try:
            user = await bot.get_entity(match)
