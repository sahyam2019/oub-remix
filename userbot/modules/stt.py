...
async def quotess(qotli):
    if transcriber.fwd_from:
        return
    if not transcriber.reply_to_msg_id:
        return await transcriber.edit("```Reply to any user message.```")
    reply_message = await transcriber.get_reply_message()
    if not reply_message.voice:
        return await transcriber.edit("```Reply to text message```")
    chat = "@transcriber_bot"
    if reply_message.sender.bot:
        return await transcriber.edit("```Reply to actual voice message.```")
    await qotli.edit("```converting```")
    try:
        async with bot.conversation(chat) as conv:
            try:
                ...
                return await qotli.reply("```Please unblock @transcriber_bot and try again```")
