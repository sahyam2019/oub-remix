...
@register(outgoing=True, pattern="^.rar(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            patoolib.create_archive(directory_name + ".rar",(directory_name,TEMP_DOWNLOAD_DIRECTORY))
...
@register(outgoing=True, pattern="^.7z(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            # patoolib.create_archive(directory_name + '.7z',directory_name)
            patoolib.create_archive(directory_name + ".7z",(directory_name,TEMP_DOWNLOAD_DIRECTORY))
            # patoolib.create_archive("/content/21.yy Avrupa (1).pdf.zip",("/content/21.yy Avrupa (1).pdf","/content/"))
            await bot.send_file(
                event.chat_id,
                directory_name + ".7z",
...
@register(outgoing=True, pattern="^.unzipper(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
